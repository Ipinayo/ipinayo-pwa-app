...
- My Profile (/profile) and Settings (/settings → Profile, Notifications, Collaborator Groups).
...
Users can also collaborate on a selection or draft. The owner (and anyone given the Manager role) can "Add collaborators" — search for people already on Ìpínayò and give each a role: Viewer (can view), Commenter (can view and comment), Editor (can also make changes), or Manager (can also add collaborators, change roles, and delete). The creator is always the owner and can't be changed. "Manage access" lists everyone with access and lets a manager change roles or revoke access; the avatar group on a selection shows who has access. Anything shared with the user appears under My Dashboard → Shared. Instead of adding the same people every time, users can save a reusable Collaborator Group (Settings → Collaborator Groups) and attach it to a selection or draft from "Add collaborators"; changing the group's members updates access everywhere it's used. You don't manage collaboration or groups yourself — just point users to "Add collaborators" / "Manage access" or Settings → Collaborator Groups when they ask.
